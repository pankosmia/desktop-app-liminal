...
| Description | Linux | Windows | MacOS |
|-------|-------|---------|-------|
| Facilitates syncing with upstream with exclusion of files expected to differ: | /linux/scripts/sync.bsh | \windows\scripts\sync.bat<br />Optional arguments:<br />`.\sync.bat -p`<br />(Will not ask if the latest is already pulled.) | /macos/scripts/sync.zsh |